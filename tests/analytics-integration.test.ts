...
describe("Analytics report — real DB end to end", () => {
  ...
  it("computes real metrics/insights/recommendations from real persisted activity", async () => {
    ...
    const r1 = db.createReminder(userId, { title: "Water plants", date: "2026-01-01", time: "08:00", priority: "medium", types: ["general"] });
    db.completeReminder(r1.id);
    const r2 = db.createReminder(userId, { title: "Call dentist", date: "2026-01-01", time: "10:00", priority: "medium", types: ["call"] });
    db.completeReminder(r2.id);
    db.createReminder(userId, { title: "Renew license", date: "2026-01-01", time: "10:00", priority: "low", types: ["general"] });
    db.createReminder(userId, { title: "Read book", date: "2026-01-01", time: "10:00", priority: "low", types: ["general"] });
    db.createReminder(userId, { title: "Clean garage", date: "2026-01-01", time: "10:00", priority: "low", types: ["general"] });

    // A reminder snoozed repeatedly.
    const snoozed = db.createReminder(userId, { title: "Take vitamins", date: "2026-01-01", time: "09:00", priority: "medium", types: ["general"] });
    for (let i = 0; i < 3; i++) db.snoozeReminder(snoozed.id, 15);
    ...
    const paymentReminder = db.createReminder(userId, {
    ...
    db.linkPaymentCycleReminder(cycle.id, paymentReminder.id);
    const occ = db.addOccurrence(paymentReminder.id, "2026-01-10T09:00:00.000Z");
    db.logNotification({ reminderId: paymentReminder.id, occurrenceId: occ.id, channel: "sms", message: "Pay your bill", outcome: "failed" });
    db.logNotification({ reminderId: paymentReminder.id, occurrenceId: occ.id, channel: "push", message: "Pay your bill", outcome: "sent" });
    ...
    const snoozedReminderAfterReport = db.getReminder(snoozed.id)!;
    ...
    const beforeTime = db.getReminder(snoozed.id)!.time;
    applyRecommendation(rec!.id);
    const afterTime = db.getReminder(snoozed.id)!.time;
    ...
    const secondReport = buildAnalyticsReport(userId);
    expect(secondReport.recommendations.find((r) => r.subjectId === snoozed.id)).toBeUndefined();
    const states = db.listRecommendationStates(userId);
