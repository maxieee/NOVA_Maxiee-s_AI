...
describe("Automation engine — triggers, idempotency, anti-chaining", () => {
  ...
  it("creates, lists, enables/disables and deletes an automation via DataLayer", async () => {
    ...
    const created = await db.createAutomation(userId, {
    ...
    expect(await db.listAutomations(userId)).toHaveLength(1);

    const disabled = await db.updateAutomation(created.id, { enabled: false });
    expect(disabled?.enabled).toBe(false);

    await db.deleteAutomation(created.id);
    expect(await db.listAutomations(userId)).toHaveLength(0);
  });
  ...
  it("fires a reminder_completed automation exactly once and creates a real follow-up reminder", async () => {
    ...
    await db.createAutomation(userId, {
    ...
    const completed = (await db.completeReminder(reminder.id))!;

    const before = (await db.listReminders(userId)).length;
    const results = await onReminderCompleted(completed);
    const after = await db.listReminders(userId);
  ...
  it("does not fire for an unrelated reminder completion when a title condition doesn't match", async () => {
    ...
    const unrelated = await db.createReminder(userId, {
      title: "Call mom",
      date: "2026-09-18",
      priority: "low",
      types: ["general"],
    });
    const completed = (await db.completeReminder(unrelated.id))!;

    const before = (await db.listReminders(userId)).length;
    const results = await onReminderCompleted(completed);
    const after = (await db.listReminders(userId)).length;
  ...
  it("is idempotent: replaying the same completion event does not double-fire", async () => {
    ...
    const countAfterFirst = (await db.listReminders(userId)).length;

    // Simulate the same completion event being delivered a second time.
    const second = await onReminderCompleted(completed);
    const countAfterSecond = (await db.listReminders(userId)).length;
  ...
  it("prevents automation chaining: completing an automation-created reminder never fires a new automation", async () => {
    ...
    const source = await db.createReminder(userId, {
      title: "Original task",
      date: "2026-09-18",
      priority: "medium",
      types: ["general"],
    });
    const completedSource = (await db.completeReminder(source.id))!;
    await onReminderCompleted(completedSource);

    const followUp = (await db.listReminders(userId)).find((r) => r.title === "Check back in")!;
    expect(followUp).toBeTruthy();

    const totalBefore = (await db.listReminders(userId)).length;
    const completedFollowUp = (await db.completeReminder(followUp.id))!;
    const chainResults = await onReminderCompleted(completedFollowUp);
    const totalAfter = (await db.listReminders(userId)).length;
  ...
  it("fires a periodic (cron_daily) automation once per day and is idempotent across repeated evaluations", async () => {
    ...
    expect((await db.listReminders(userId)).length).toBe(countAfterFirst + 1);
  });
  ...
  it("action calls the real existing reminder-creation path (createReminder), not a parallel executor", async () => {
    ...
    const created = (await db.listReminders(userId)).find((r) => r.title === "Verify real record");
