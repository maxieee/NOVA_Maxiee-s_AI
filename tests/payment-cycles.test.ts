...
describe("payment_accounts / payment_cycles — DataLayer + cycle generation", () => {
  ...
  it("creates a payment account with sane defaults and no duplicate-per-period cycles", async () => {
    ...
    const now = new Date();
    const cycle1 = await ensureUpcomingCycle(account, now);
    const cycle2 = await ensureUpcomingCycle(account, now);
    const cycle3 = await ensureUpcomingCycle(account, new Date(now.getTime() + 1000)); // a moment later, same period
    ...
    expect(await db.listPaymentCycles(account.id)).toHaveLength(1);
  });
  ...
  it("disabling an account stops it from being picked up by generateMissingCycles", async () => {
    ...
    await db.setPaymentAccountActive(account.id, false);
    generateMissingCycles(userId);
    expect(await db.listPaymentCycles(account.id)).toHaveLength(0);
  });
  ...
  it("Mark Paid is idempotent and completes the linked reminder via the existing completeReminder path", async () => {
    ...
    expect(cycleAfterLink?.reminder_id).toBe(reminder!.id);

    const paid1 = await db.markPaymentCyclePaid(cycle.id);
    expect(paid1?.status).toBe("paid");
    const reminderAfterPaid = await db.getReminder(reminder!.id);
    expect(reminderAfterPaid?.status).toBe("completed");

    // Idempotent: a second Mark Paid call is a no-op, not an error.
    const paid2 = await db.markPaymentCyclePaid(cycle.id);
  ...
  it("generateReminderForCycle is idempotent per cycle (re-running never creates a second reminder)", async () => {
    ...
    const cycle = await ensureUpcomingCycle(account);
    const r1 = await generateReminderForCycle(cycle, account, userId, ["push"], "normal");
    ...
    const remindersForUser = await db.listReminders(userId, { types: ["payment"] });
    expect(remindersForUser.filter((r) => r.id === r1!.id)).toHaveLength(1);
    const cycleAfterGen = await db.getPaymentCycle(cycle.id);
    expect(cycleAfterGen?.reminder_id).toBe(r1!.id);
  });
});
