...
async function main() {
  ...
  const reminder = generateReminderForCycle(cycle, account, userId, preferences.preferred_channels, preferences.default_intensity)!;
  console.log("reminder generated:", reminder.id, "occurrences:", db.listOccurrences(reminder.id).length);
  console.log("cycle now linked to reminder_id:", db.getPaymentCycle(cycle.id)?.reminder_id === reminder.id);

  console.log("\n" + "=".repeat(20), "STEP 3: backdate the earliest occurrence so it's due now", "=".repeat(20));
  const Database = (await import("better-sqlite3")).default;
  const sqlite = new Database(DB_PATH);
  const earliestOcc = db.listOccurrences(reminder.id)[0];
  sqlite
  ...
  console.log("occurrence:", occSnapshot(db.listOccurrences(reminder.id)[0]));
  ...
  const paid = db.markPaymentCyclePaid(cycle.id);
  console.log("cycle status:", paid?.status, "paid_at:", paid?.paid_at);
  console.log("reminder status:", db.getReminder(reminder.id)?.status);
  console.log("occurrence after Mark Paid:", occSnapshot(db.listOccurrences(reminder.id)[0]));

  const notificationsBefore = db.listNotifications(reminder.id).length;
  await sleep(1000);
  await cronOnce(userId);
  const notificationsAfter = db.listNotifications(reminder.id).length;
  ...
  const cyclesForAccount = db.listPaymentCycles(account.id);
  console.log(`cycles for account after 10 more cron runs: ${cyclesForAccount.length} (must stay 1 until the next real period)`);
  console.log(`notifications for reminder after 10 more cron runs: ${db.listNotifications(reminder.id).length} (must equal ${notificationsAfter})`);

  console.log("\n" + "=".repeat(20), "STEP 6: multi-account idempotency", "=".repeat(20));
  const account2 = db.createPaymentAccount(userId, {
    name: "Gym EMI",
    payment_type: "EMI",
    issuer: null,
  ...
  console.log(
    "account2 cycles:",
    db.listPaymentCycles(account2.id).length,
